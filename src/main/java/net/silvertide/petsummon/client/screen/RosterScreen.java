 ...
 *
 * Polish (palette, animations, scrolling, validation tooltips, entity preview)
 * is layered on later.
 */
public final class RosterScreen extends Screen {
    ...
    protected void init() {
        super.init();
        // Up to ~6 visible rows + header/footer.
        panelHeight = Math.min(this.height - 40, 6 * ROW_HEIGHT + 40);
        leftPos = (this.width - PANEL_WIDTH) / 2;
        topPos = (this.height - panelHeight) / 2;
        rowsTop = topPos + 24;
        rowsBottom = topPos + panelHeight - 8;

        // Refresh roster on open. Server responds with S2CRosterSync, which updates
        // ClientRosterData; render() reads from there each frame.
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        if (bonds.isEmpty()) {
            g.drawCenteredString(font, Component.translatable("petsummon.screen.empty"),
                    leftPos + PANEL_WIDTH / 2, topPos + panelHeight / 2 - 4, C_TEXT_MUTED);
            return;
        }
    ...
    private void renderRow(GuiGraphics g, BondView bond, int x, int y, int w, int mx, int my) {
        ...
        // Name (or fallback to entity-type path)
