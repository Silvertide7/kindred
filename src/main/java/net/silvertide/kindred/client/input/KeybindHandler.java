    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        while (Keybinds.SUMMON_ACTIVE_PET.consumeClick()) {
            ...
            if (ClientRosterData.bonds().isEmpty()) {
                showActionBar("kindred.summon.no_bonds");
                continue;
            }
            Optional<BondView> target = ClientRosterData.findActive();
            if (target.isEmpty()) {
                showActionBar("kindred.summon.no_active");
                continue;
            }
            if (ClientRosterData.isRevivalPending(target.get())) {
                showActionBar("kindred.summon.reviving");
                continue;
            }
            if (ClientRosterData.isGlobalSummonOnCooldown()) {
                showActionBar("kindred.summon.global_cooldown");
                continue;
            }
            if (ClientRosterData.isOnCooldown(target.get())) {
                showActionBar("kindred.summon.on_cooldown");
                continue;
            }
