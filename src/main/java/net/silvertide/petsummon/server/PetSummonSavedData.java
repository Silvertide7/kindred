...
public class PetSummonSavedData extends SavedData {
    public static final String FILE_ID = "petsummon_data";
    ...
    private PetSummonSavedData() {
        this(new HashMap<>(), new LinkedHashSet<>(), new LinkedHashSet<>(), new HashMap<>());
    }

    private PetSummonSavedData(Map<UUID, Integer> revisions, Set<UUID> pending, Set<UUID> killed, Map<UUID, OfflineSnapshot> snapshots) {
    ...
    public static SavedData.Factory<PetSummonSavedData> factory() {
        return new SavedData.Factory<>(PetSummonSavedData::new, PetSummonSavedData::load, null);
    }

    public static PetSummonSavedData get(ServerLevel level) {
        return level.getServer().overworld().getDataStorage().computeIfAbsent(factory(), FILE_ID);
    }

    public static PetSummonSavedData load(CompoundTag tag, HolderLookup.Provider lookup) {
        Optional<Snapshot> parsed = Snapshot.CODEC.parse(NbtOps.INSTANCE, tag)
                .resultOrPartial(err -> PetSummon.LOGGER.error("Failed to load {}: {}", FILE_ID, err));
        if (parsed.isEmpty()) return new PetSummonSavedData();
        Snapshot s = parsed.get();
        return new PetSummonSavedData(
    ...
    public CompoundTag save(CompoundTag tag, HolderLookup.Provider lookup) {
        ...
        Snapshot.CODEC.encodeStart(NbtOps.INSTANCE, s)
                .resultOrPartial(err -> PetSummon.LOGGER.error("Failed to save {}: {}", FILE_ID, err))
